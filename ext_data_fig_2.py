...
def ext_data_fig_2AB():
	...
	plt.savefig('./figurePanels/ext_data_2A.png', dpi = 800)
	plt.savefig('./figurePanels/ext_data_2A.pdf', dpi = 800)
	plt.close()
	...
	_=plt.yticks([])
	plt.legend([],[], frameon=False)
	plt.savefig('./figurePanels/ext_data_2B.png', dpi = 800)
	plt.savefig('./figurePanels/ext_data_2B.pdf', dpi = 800)
	plt.close()
...
def ext_data_fig_2C():
	...
	plt.savefig(f"./figurePanels/ext_data_2C.png", dpi = 800, bbox_inches='tight')
	plt.savefig(f"./figurePanels/ext_data_2C.pdf", dpi = 800, bbox_inches='tight')


def ext_data_fig_2D():

	metabs = get_metabs(min_impute=False, named_only=True)
	md = get_md()
	...
	curr_metabs = ['glycerophosphoserine*', 'choline', 'spermine', 
					...
					'tartarate', 'ethyl beta-glucopyranoside', 'diethanolamine']
	...
	plt.xticks([], [])
	plt.yticks([], [])
	plt.savefig("figurePanels/ext_data_2D.pdf", dpi=800, bbox_inches='tight')
