...
from utils.accuracy_evaluation import eval_nest, eval_nest_rs, eval_external, eval_benchmark, eval_benchmark_cat, \
    eval_race_val, eval_ns_to_s
from utils.prediction_figures import main_auroc, main_auprc, gw_thresh, external_auroc, external_auprc, micro_shap, \
    metab_shap, combo_shap, \
    SV_LR_comparison, race_strat_compare_to_non_strat, model_performance_by_race, batch_auroc, compare_aucs
from utils.get_dfs import get_md, get_sample_batches, get_metabs, get_cid_di, get_usearch_otus, get_clin_df_for_pred, \
    get_ghartey_2014, get_ghartey_2016
...
def main():
    ...
    if not os.path.exists("data/age_bmi.csv"):
        print("Missing age and bmi data, will not train or evaluate clinical or combination models")
    if os.path.exists("data/2014_v2.XLSX") and os.path.exists("data/2016_v2.XLSX"):
        pass
    else:
        print("Missing validation (Ghartey) datasets, will not run external validation for metabolomics model")
    ...
    if os.path.exists("data/age_bmi.csv"):
        combination = pd.concat(
            [clinical.add_suffix("_clin"), microbiome.add_suffix('_micro'), metabolomics.add_suffix('_metab')], axis=1)
    ...
    if os.path.exists("data/age_bmi.csv"):
        data_list = [clinical, microbiome_AA, microbiome_non_AA, metabolomics_AA, metabolomics_non_AA, combination_AA,
                     combination_non_AA]
        ...
        data_list = [microbiome_AA, microbiome_non_AA, metabolomics_AA, metabolomics_non_AA]
        md_list = [metadata_AA.copy(), metadata_non_AA.copy(), metadata_AA.copy(), metadata_non_AA.copy()]

    data_benchmark_list = [metabolomics_AA, metabolomics_non_AA, metabolomics_AA, metabolomics_non_AA, metabolomics_AA,
                           metabolomics_non_AA,
                           metabolomics_AA, metabolomics_non_AA, metabolomics]
    md_benchmark_list = [metadata_AA.copy(), metadata_non_AA.copy(), metadata_AA.copy(),
                         metadata_non_AA.copy(), metadata_AA.copy(), metadata_non_AA.copy(),
                         metadata_AA.copy(), metadata_non_AA.copy(), metadata.copy()]
    ...
    clinical_cv = [pd.read_pickle(i) for i in
                   sorted(glob.glob(join('ml_models/clinical_nested', 'nested_structure/*')))]

    microbiome_AA_ehps_list = [pd.read_pickle('ml_models/microbiome_nested/AA/best_models/' + str(j + 1)) for j in
                               range(50)]
    microbiome_AA_cv = [pd.read_pickle(i) for i in
                        sorted(glob.glob(join('ml_models/microbiome_nested/AA', 'nested_structure/*')))]
    microbiome_AA_shap_model = pd.read_pickle('ml_models/shap_models/micro_AA')
    microbiome_non_AA_ehps_list = [pd.read_pickle('ml_models/microbiome_nested/non_AA/best_models/' + str(j + 1)) for j
                                   in range(50)]
    microbiome_non_AA_cv = [pd.read_pickle(i) for i in
                            sorted(glob.glob(join('ml_models/microbiome_nested/non_AA', 'nested_structure/*')))]
    microbiome_non_AA_shap_model = pd.read_pickle('ml_models/shap_models/micro_non_AA')

    metabolomics_AA_ehps_list = [pd.read_pickle('ml_models/metabolomics_nested/AA/best_models/' + str(j + 1)) for j in
                                 range(50)]
    metabolomics_AA_cv = [pd.read_pickle(i) for i in
                          sorted(glob.glob(join('ml_models/metabolomics_nested/AA', 'nested_structure/*')))]
    metabolomics_AA_shap_model = pd.read_pickle('ml_models/shap_models/metab_AA')
    metabolomics_non_AA_ehps_list = [pd.read_pickle('ml_models/metabolomics_nested/non_AA/best_models/' + str(j + 1))
                                     for j in range(50)]
    metabolomics_non_AA_cv = [pd.read_pickle(i) for i in
                              sorted(glob.glob(join('ml_models/metabolomics_nested/non_AA', 'nested_structure/*')))]
    metabolomics_non_AA_shap_model = pd.read_pickle('ml_models/shap_models/metab_non_AA')

    combination_AA_ehps_list = [pd.read_pickle('ml_models/combination_nested/AA/best_models/' + str(j + 1)) for j in
                                range(50)]
    combination_AA_cv = [pd.read_pickle(i) for i in
                         sorted(glob.glob(join('ml_models/combination_nested/AA', 'nested_structure/*')))]
    combination_AA_shap_model = pd.read_pickle('ml_models/shap_models/combo_AA')
    combination_non_AA_ehps_list = [pd.read_pickle('ml_models/combination_nested/non_AA/best_models/' + str(j + 1)) for
                                    j in range(50)]
    combination_non_AA_cv = [pd.read_pickle(i) for i in
                             sorted(glob.glob(join('ml_models/combination_nested/non_AA', 'nested_structure/*')))]
    combination_non_AA_shap_model = pd.read_pickle('ml_models/shap_models/combo_AA')

    if os.path.exists("data/age_bmi.csv"): ### IF WE ACTUALLY HAVE DATA ON AGE AND BMI
        ehps_list = [clinical_ehps_list, microbiome_AA_ehps_list, microbiome_non_AA_ehps_list, metabolomics_AA_ehps_list,
                     metabolomics_non_AA_ehps_list,
        ...
        shap_ehps_list = [microbiome_AA_shap_model, microbiome_non_AA_shap_model, metabolomics_AA_shap_model,
                          metabolomics_non_AA_shap_model,
                          combination_AA_shap_model, combination_non_AA_shap_model]
    else:
        ehps_list = [microbiome_AA_ehps_list, microbiome_non_AA_ehps_list,
                     metabolomics_AA_ehps_list,
                     metabolomics_non_AA_ehps_list]

        cv_list = [microbiome_AA_cv, microbiome_non_AA_cv, metabolomics_AA_cv, metabolomics_non_AA_cv]
    ...
    lightGBM_AA_cv = pd.read_pickle('ml_models/benchmark_models/benchmark_AA_cv')

    lightGBM_non_AA_hp = pd.read_pickle('ml_models/benchmark_models/benchmark_lightGBM_non_AA')
    lightGBM_non_AA_cv = pd.read_pickle('ml_models/benchmark_models/benchmark_non_AA_cv')

    SVC_AA_hp = pd.read_pickle('ml_models/benchmark_models/benchmark_SVC_AA')
    SVC_AA_cv = pd.read_pickle('ml_models/benchmark_models/benchmark_AA_cv')

    SVC_non_AA_hp = pd.read_pickle('ml_models/benchmark_models/benchmark_SVC_non_AA')
    SVC_non_AA_cv = pd.read_pickle('ml_models/benchmark_models/benchmark_non_AA_cv')

    LR_AA_hp = pd.read_pickle('ml_models/benchmark_models/benchmark_LR_AA')
    LR_AA_cv = pd.read_pickle('ml_models/benchmark_models/benchmark_AA_cv')

    LR_non_AA_hp = pd.read_pickle('ml_models/benchmark_models/benchmark_LR_non_AA')
    LR_non_AA_cv = pd.read_pickle('ml_models/benchmark_models/benchmark_non_AA_cv')

    EN_AA_hp = pd.read_pickle('ml_models/benchmark_models/benchmark_EN_AA')
    EN_AA_cv = pd.read_pickle('ml_models/benchmark_models/benchmark_AA_cv')

    EN_non_AA_hp = pd.read_pickle('ml_models/benchmark_models/benchmark_EN_non_AA')
    EN_non_AA_cv = pd.read_pickle('ml_models/benchmark_models/benchmark_non_AA_cv')

    metab_ns_hp = pd.read_pickle('ml_models/benchmark_models/benchmark_metab_ns')
    metab_ns_cv = pd.read_pickle('ml_models/benchmark_models/benchmark_ns_cv')
    ...
    benchmark_ehps_list = [lightGBM_AA_hp, lightGBM_non_AA_hp, SVC_AA_hp, SVC_non_AA_hp, LR_AA_hp, LR_non_AA_hp,
                           EN_AA_hp, EN_non_AA_hp, metab_ns_hp]
    benchmark_cv_list = [lightGBM_AA_cv, lightGBM_non_AA_cv, SVC_AA_cv, SVC_non_AA_cv, LR_AA_cv, LR_non_AA_cv, EN_AA_cv,
                         EN_non_AA_cv, metab_ns_cv]
    ...
    study_design_list = [Study_Design.NORMAL_OPTIMIZATION, Study_Design.NESTED_OPTIMIZATION,
                         Study_Design.RANDOM_SAMPLING_OPTIMIZATION,
                         Study_Design.CHECK_SHAP_IMPORTANCE, Study_Design.NESTED_VALIDATION,
                         Study_Design.EXTERNAL_VALIDATION,
                         Study_Design.NESTED_P_VALUE]
    ml_method_list = [Machine_Learning_Method.LIGHTGBM, Machine_Learning_Method.SVC, Machine_Learning_Method.LR]
    ml_method_benchmark_list = [Machine_Learning_Method.LIGHTGBM, Machine_Learning_Method.LIGHTGBM,
                                Machine_Learning_Method.SVC, Machine_Learning_Method.SVC,
                                Machine_Learning_Method.LR, Machine_Learning_Method.LR,
                                Machine_Learning_Method.EN, Machine_Learning_Method.EN,
    ...
    if exists('ml_models/res_benchmark'):
        ...
        for i in range(len(benchmark_ehps_list)):
            ...
            for j in range(len(benchmark_cv_list[i])):
                ...
                if (ml_method_benchmark_list[i] == Machine_Learning_Method.LR) | (
                        ml_method_benchmark_list[i] == Machine_Learning_Method.SVC):
                    is_classifier = True
                else:
                    is_classifier = False
    ...
    result_benchmark_list_cat = [[result_benchmark_list[0], result_benchmark_list[1]],
                                 ...
                                 [result_benchmark_list[6], result_benchmark_list[7]]]
    ...
    if exists('ml_models/res_race_val'):
        ...
        for i in range(2):
            result_race_val_list.append(pipe(X=data_benchmark_list[i].copy(),
                                             md=md_benchmark_list[i].copy(),
                                             params_list=benchmark_ehps_list[i].copy(),
                                             cv={"1_KFold_0": [list(data_benchmark_list[i].index),
                                                               list(data_race_val_list[i].index)]},
                                             classifier=False,
                                             ml_method=ml_method_list[0],
            ...
            for i in range(2):
                if os.path.exists("data/age_bmi.csv"):
                    result_validation_list.append(pipe(X=data_list[i + 3].copy(),
                                                       ...
                                                       cv={"1_KFold_0": [list(data_list[i + 3].index),
                                                                         list(data_list[i + 3].index)]},
                                                       classifier=False,
                                                       ml_method=ml_method_list[0],
                                                       study_design=study_design_list[5],
                                                       train_samples_to_remove=train_samples_to_remove,
                    ...
                    result_validation_list.append(pipe(X=data_list[i + 2].copy(),
                                                       md=md_list[i + 2].copy(),
                                                       params_list=shap_ehps_list[i + 2].copy(),
                                                       cv={"1_KFold_0": [list(data_list[i + 2].index),
                                                                         list(data_list[i + 2].index)]},
                                                       classifier=False,
                                                       ml_method=ml_method_list[0],
                                                       study_design=study_design_list[5],
                                                       train_samples_to_remove=train_samples_to_remove,
    ...
    if exists('ml_models/res_batch'):
        ...
        for i in range(2):
            result_batch_list.append(pipe(X=data_batch_list[i].copy(),
                                          md=md_batch_list[i].copy(),
                                          params_list=batch_ehps_list[i].copy(),
                                          cv={"1_KFold_0": [list(data_batch_list[i].index),
                                                            list(data_batch_list[-i].index)]},
                                          classifier=False,
                                          ml_method=ml_method_list[0],
                                          study_design=study_design_list[5],
                                          train_samples_to_remove=train_samples_to_remove,
        ...
        if os.path.exists("data/age_bmi.csv"):
            for i in range(6):
                result_shap_list.append(pipe(X=data_list[i + 1].copy(),
                                             ...
                                             train_samples_to_remove=[]))
        else:
            for i in range(4):
                result_shap_list.append(pipe(X=data_list[i].copy(),
                                             md=md_list[i].copy(),
                                             params_list=shap_ehps_list[i].copy(),
                                             cv={"1_KFold_0": [list(data_list[i].index),
                                                               list(data_list[i].index)]},
                                             classifier=False,
                                             ml_method=ml_method_list[0],
                                             study_design=study_design_list[3],
                                             train_samples_to_remove=[]))
    ...
    if os.path.exists("data/age_bmi.csv"):
        ...
        micro_28, micro_32, micro_37, micro_pred = eval_nest_rs([result_robust_list[0], result_robust_list[1]])
        micro_28_AA, micro_32_AA, micro_37_AA, micro_AA_pred = eval_nest(result_robust_list[0])
        micro_28_non_AA, micro_32_non_AA, micro_37_non_AA, micro_non_AA_pred = eval_nest(result_robust_list[1])

        metab_28, metab_32, metab_37, metab_pred = eval_nest_rs([result_robust_list[2], result_robust_list[3]])
        metab_28_AA, metab_32_AA, metab_37_AA, metab_AA_pred = eval_nest(result_robust_list[2])
        metab_28_non_AA, metab_32_non_AA, metab_37_non_AA, metab_non_AA_pred = eval_nest(result_robust_list[3])


    y_test_all, y_pred_all, auROC_all = eval_benchmark(result_benchmark_list,
                                                       [True, True, False, False, False, False, True, True, True])

    ## Order it LGB, SVC, LR, and EN
    ...
    auROC_met6_AA, auROC_met6_non_AA = eval_ns_to_s(result_benchmark_list[8], list(metadata_AA.index),
                                                    list(metadata_non_AA.index))
    ...
    if os.path.exists("data/age_bmi.csv"):
        ...
        test_list_all_28 = [micro_28, metab_28]
        test_list_all_32 = [micro_32, metab_32]
        test_list_all_37 = [micro_37, metab_37]
        pred_list_all = [micro_pred, metab_pred]
    ...
    if os.path.exists("data/age_bmi.csv"):
        auroc_list_all_37 = [[roc_auc_score(test_list_all_37[j][i], pred_list_all[j][i]) for i in range(5)] for j in
                             range(4)]
        auroc_list_all_37_AA = [[roc_auc_score(test_list_all_37_AA[j][i], pred_list_all_AA[j][i]) for i in range(5)] for j
                                in range(2)]
        auroc_list_all_32 = [[roc_auc_score(test_list_all_32[j][i], pred_list_all[j][i]) for i in range(5)] for j in
                             range(4)]
        auroc_list_all_32_AA = [[roc_auc_score(test_list_all_32_AA[j][i], pred_list_all_AA[j][i]) for i in range(5)] for j
                                in range(2)]
        auroc_list_all_28 = [[roc_auc_score(test_list_all_28[j][i], pred_list_all[j][i]) for i in range(5)] for j in
                             range(4)]
        auroc_list_all_28_AA = [[roc_auc_score(test_list_all_28_AA[j][i], pred_list_all_AA[j][i]) for i in range(5)] for j
                                in range(2)]
        ...
        auprc_list_all = [[average_precision_score(test_list_all_37[j][i], pred_list_all[j][i]) for i in range(5)] for j in
                          range(4)]
    else:
        auroc_list_all_37 = [[roc_auc_score(test_list_all_37[j][i], pred_list_all[j][i]) for i in range(5)] for j in
                             range(2)]
    ...
    '''
    Generate out ROC-ROC p values
    '''
    print("Generating the ROC-ROC p-values")

    if os.path.exists("data/age_bmi.csv"):
        print("\nClinical 37 vs Microbiome 37:")
        compare_aucs(auroc_list_all_37[0], auroc_list_all_37[1])
        print("\nClinical 37 vs Metabolomics 37:")
        compare_aucs(auroc_list_all_37[0], auroc_list_all_37[2])
        print("\nMicrobiome 37 vs Metabolomics 37:")
        compare_aucs(auroc_list_all_37[1], auroc_list_all_37[2])
        print("\nCombintation 37 vs Metabolomics 37:")
        compare_aucs(auroc_list_all_37[3], auroc_list_all_37[2])

        print("\nMetabolome 37 vs Metabolome 32:")
        compare_aucs(auroc_list_all_32[2], auroc_list_all_37[2])
        print("\nMetabolome 37 vs Metabolomics 28:")
        compare_aucs(auroc_list_all_28[2], auroc_list_all_37[2])

        print("\nMicrobiome 37 vs Microbiome 32:")
        compare_aucs(auroc_list_all_32[1], auroc_list_all_37[1])
        print("\nMicrobiome 37 vs Microbiome 28:")
        compare_aucs(auroc_list_all_28[1], auroc_list_all_37[1])
    else:
        print("\nMicrobiome 37 vs Metabolomics 37:")
        compare_aucs(auroc_list_all_37[0], auroc_list_all_37[1])
        ...
        print("\nMicrobiome 37 vs Microbiome 32:")
        compare_aucs(auroc_list_all_32[0], auroc_list_all_37[0])
        print("\nMicrobiome 37 vs Microbiome 28:")
        compare_aucs(auroc_list_all_28[0], auroc_list_all_37[0])

    print("\nLightGBM 37 vs SVC 37:")
    compare_aucs(auROC_all_cat[0], auROC_all_cat[1])
    print("\nLightGBM 37 vs Logistic Regression 37:")
    compare_aucs(auROC_all_cat[0], auROC_all_cat[2])
    print("\nLightGBM 37 vs Elastic Net 37:")
    compare_aucs(auROC_all_cat[0], auROC_all_cat[3])

    print("\nnon race strat eval on non-AA (CV) vs non-AA on non-AA (CV):")
    compare_aucs(auROC_r[1], auROC_r[3])

    '''
    Generate the figures
    '''
    print("Generating the figures...")
    main_auroc(test_list_all_37, pred_list_all, auroc_list_all_37, False, age_bmi=os.path.exists("data/age_bmi.csv"))
    main_auprc(test_list_all_37, pred_list_all, auprc_list_all, False, age_bmi=os.path.exists("data/age_bmi.csv"))

    if os.path.exists("data/age_bmi.csv"): ## only make the metabolomics - combination comparison panels if we can run the combination model
        main_auroc([test_list_all_37[2], test_list_all_37[3]], [pred_list_all[2], pred_list_all[3]],
                   [auroc_list_all_37[2], auroc_list_all_37[3]], True)
        main_auprc([test_list_all_37[2], test_list_all_37[3]], [pred_list_all[2], pred_list_all[3]],
                   [auprc_list_all[2], auprc_list_all[3]], True)
    ...
    batch_auroc(random_sampling_acc, statistics.mean(batch_auROC_all[0]), statistics.mean(batch_auROC_all[1]),
                batch_1_val, batch_2_val)
