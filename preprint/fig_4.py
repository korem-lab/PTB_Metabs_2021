...
def figure_4ABC():
	...
	### Concatenate stratified model_1
	test_list_all = []
	pred_list_all = []
	auroc_list_all = []
	...
	plt.savefig('figurePanels/4A.png', dpi=800)
	plt.savefig('figurePanels/4A.pdf', dpi=800)
	plt.close()
	...
	#plt.title("B. sPTB prediction accuracy (auPR)", weight='bold', fontsize=9)
	plt.savefig('figurePanels/4B.png', dpi=800)
	plt.savefig('figurePanels/4B.pdf', dpi=800)
	plt.close()

	################### 4C #############################
	stratify = [True, True, True]
	...
	for i in range(len(gw_list)): # per dataset
	    ...
	    if stratify[i]:
	        for j in range(len(gw_best_y_test_all_auroc[2][0][0])): # per cv
	            test = np.concatenate([gw_best_y_test_all_auroc[i][1][0][j], gw_best_y_test_all_auroc[i][1][1][j]])
	            pred = np.concatenate([gw_best_y_pred_all_auroc[i][1][0][j], gw_best_y_pred_all_auroc[i][1][1][j]])
	            test_list.append(test)
	            pred_list.append(pred)
	            auroc_list.append(roc_auc_score(test, pred))
	        ...
	        for j in range(len(ns_gw_best_y_test_all_auroc[2][0][0])): # per cv
	            test = ns_gw_best_y_test_all_auroc[i][1][0][j]
	            pred = ns_gw_best_y_pred_all_auroc[i][1][0][j]
	            test_list.append(test)
	            pred_list.append(pred)
	            auroc_list.append(roc_auc_score(test, pred))
	...
	#plt.title("B. Microbiome Severe sPTB prediction", weight='bold', fontsize=8)
	plt.legend(loc='lower right', fontsize=5)
	plt.savefig('figurePanels/4C.png', dpi=800)
	plt.savefig('figurePanels/4C.pdf', dpi=800)
	plt.close()
...
def figure_4D():
	...
	fpr_1, tpr_1, thresholds_1 = metrics.roc_curve(b_16_y_test, b_16_y_pred, drop_intermediate=False)
	auc_1 = roc_auc_score(b_16_y_test, b_16_y_pred)
	fpr_2, tpr_2, thresholds_2 = metrics.roc_curve(w_14_y_test, w_14_y_pred, drop_intermediate=False)
	auc_2 = roc_auc_score(w_14_y_test, w_14_y_pred)

	c=['olive', 'Teal']

	plt.figure(figsize=(2.7,2.7))
	lw = 2
	plt.plot(fpr_1, tpr_1, color=c[0], lw=lw, linestyle = '-', label='Ghartey 2015 (auROC = %0.2f)' % auc_1)
	plt.plot(fpr_2, tpr_2, color=c[1], lw=lw, linestyle = '-', label='Ghartey 2017 (auROC = %0.2f)' % auc_2)
	...
	#plt.ylabel('True Positive Rate', fontsize=15)
	plt.legend(loc="lower right", fontsize=5)
	plt.savefig('figurePanels/4D.pdf', dpi=800)
	plt.savefig('figurePanels/4D.png', dpi=800)
	plt.close()

def figure_4E():

	# read shap required files
	res_AA = pd.read_pickle("final_models/shap/elo_False_1_bolo_AA_shap.pkl")
	res_non_AA = pd.read_pickle("final_models/shap/elo_False_1_bolo_non_AA_shap.pkl")

	full_AA = pd.read_csv("final_models/shap/x_train__3_5_guanylylcytidine_173.csv", index_col=0)
	full_non_AA = pd.read_csv("final_models/shap/x_train__3_5_guanylylcytidine_51.csv", index_col=0)
	...
	f = plt.gcf()
	f.savefig('figurePanels/4E.png', dpi=800, figsize = (5,5), bbox_inches = 'tight')
	f.savefig('figurePanels/4E.pdf', dpi=800, figsize = (5,5), bbox_inches = 'tight')
